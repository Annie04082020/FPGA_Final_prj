...
### 專題報告 [邊緣特徵提取與素描風格即時轉換](https://canva.link/8higd3gjuino2wl)

## 目錄結構說明

專案目錄下包含三個版本的 HLS 程式碼：
*   **`hls_gaussian_sketch/`**：【終極進階版】實作了 $5 \times 5$ Gaussian Blur 與 Color Dodge 演算法，能呈現出帶有陰影與漸層效果的真實鉛筆素描。**這是期末專題的最高分殺手鐧，強烈建議使用此版本！**
*   **`hls_real/`**：【基礎版】使用 $3 \times 3$ Sobel 邊緣偵測與顏色反轉（Day 2 & Day 3 進度），效果較為生硬，僅有死板的輪廓線。
*   **`hls_dummy/`**：【空殼版】僅包含 AXI 介面宣告與單純的記憶體複製（Day 1 進度），用來給系統端提早進行 PYNQ 環境建置與通訊測試。

---

## 核心檔案說明 (`hls_gaussian_sketch/` 資料夾)
...
*   **記憶體緩衝架構**：使用自訂的 Line Buffer 與 Window Buffer，確保每次運算 $5 \times 5$ 卷積時，只要從外部 DDR 讀取「1個畫素」，大幅降低記憶體頻寬消耗。
*   **硬體極致最佳化**：
    *   `#pragma HLS ARRAY_PARTITION`：將 Window 陣列打碎成暫存器，解決 BRAM 讀取衝突。
    *   `#pragma HLS PIPELINE II=1`：指示編譯器將雙重迴圈完全管線化。
*   **演算法實作**：實作零時延之 $5 \times 5$ 高斯模糊 (權重總和為 256，利用 `>> 8` 達成硬體無損除法) 與 Color Dodge 演算法，將平滑陰影漸層帶入鉛筆素描效果。
...
*   **自動驗證邏輯**：掃描邊界區域驗證 Color Dodge 邊緣產生深色線條 (像素值 < 250)，確保 FPGA 素描效果與軟體預期一致，避免過度嚴苛導致 Cosimulation 誤判。
...
2. 將 `hls_gaussian_sketch/sketch_filter.cpp` 與 `hls_gaussian_sketch/sketch_filter.h` 加入 Source files。
3. 將 `hls_gaussian_sketch/sketch_filter_tb.cpp` 加入 Testbench files。
